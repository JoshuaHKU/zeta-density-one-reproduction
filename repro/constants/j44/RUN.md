# RUN.md -- {4,4}  (repro/constants/j44)

Exact value **23/4536** = 0.0050705467,
7 dihedral orbits on the 8-cycle, {4,4}.
...
python3 ../../engines/run_class.py '{4,4}' --jobs 24
```

**--jobs ceiling 24**: 1082^2/quotient terms in R^7; memo ~0.6 GB/worker.
...
backend gmpy2 ...
  O0 size 8 ...
  total = 23/4536 = 0.0050705467
```
Expected wall clock: ~3 h on 24 cores.
...
```bash
LADDER_GPU=1 OMP_NUM_THREADS=1 python3 ../../engines/ladder_joints_sw.py 44 0.1
LADDER_GPU=1 OMP_NUM_THREADS=1 python3 ../../engines/ladder_joints_sw.py 44 0.05
python3 ../../gates/g_vjoints.py
```
h^2 Richardson `(4 L(dv/2) - L(dv))/3` must match all 7 orbits within 3e-6.
