...
# --- arithmetic backend -------------------------------------------------
# The whole inner kernel is elementwise (maximum/minimum/clip/abs/sum) over a
# dense midpoint grid: no branching, no exact arithmetic, no memoisation.
# That is a natural GPU workload, and CuPy is API-compatible here, so the
# backend is switchable.  LADDER_GPU=<device index> selects a CUDA device;
# use the V100 (fp64 1:2), NOT a GeForce card (fp64 1:32) -- the F-V-JOINTS
# tolerance is 3e-6 and the h^2 extrapolation needs fp64 grid sums.
import os as _os
_GPU = _os.environ.get("LADDER_GPU")
if _GPU is not None:
    import cupy as np
    np.cuda.Device(int(_GPU)).use()
    BACKEND = f"cupy(dev{_GPU})"
else:
    import numpy as np
    BACKEND = "numpy"
...
CLASSES = {"224": (8, (2, 2, 4)), "44": (8, (4, 4)),
           "62": (8, (2, 6)), "52": (7, (2, 5)),
           # r141: {2^5} on the 10-cycle -- the Sigma_10 component whose
           # second_path was PENDING.  All blocks are pairs, so every block
           # contributes the monomial |v| and the ambient is only R^5.
           "2p5": (10, (2, 2, 2, 2, 2)),
           "225": (9, (2, 2, 5))}
